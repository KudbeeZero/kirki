...
- [x] **trading-service** — order engine implemented: validate vs cash/holdings, atomic market fill at live price, resting limit orders matched on ticks (buy ≤ limit / sell ≥ limit), weighted-average cost basis, realised PnL on sells, immutable ledger writes (orders+positions+transactions) under row locks, publishes `order.filled`/`trade.executed`. **9 unit tests** for fill math green. Fixed per-service pattern (ioredis named import, `@types/pg`, `NestExpressApplication`, ts-jest→CJS config, typed pub/sub handler) — **builds clean**. NOTE: required reconciling the schema — `market_data`/`positions`/`orders`/`transactions` keyed off `symbol` (matching the source-of-truth types) instead of the never-wired `market_id`.
- [x] **portfolio-service** — read model over the shared tables: mark-to-market totals + PnL%, per-position unrealized PnL, stats (win rate over *closed* trades, best/worst, realized PnL). Decided trading-service is the single writer (atomic settle), so dropped the double-settling `applyTrade`/event-consumer that would have corrupted balances. Persisted `realized_pnl` on sell fills (schema + engine) so stats are real. **6 unit tests**; builds clean (per-service fix pattern applied).
...
- 2026-06-09 (trading-loop 1) — **portfolio-service** implemented as a read model (mark-to-market, PnL%, unrealized PnL, win-rate stats); persisted `realized_pnl` on sells (schema + trading engine); decided trading-service = single writer (removed double-settle). 6 tests; builds clean. Pipeline green: 15/15 tasks, **90 tests**. New branch `claude/simcoin-trading-loop`; added `CLAUDE.md` session memory.
- 2026-06-09 (review+repair) — Fixed broken `@simcoin/sdk` build (missing DOM lib) that was cascading and failing the whole `pnpm test`/CI pipeline. Implemented the **trading-service order engine** end-to-end + 9 fill-math tests. Reconciled the trading-domain schema (`symbol` keys, matching the types) so persistence actually works. Hardened auth JWT verify (pinned algorithm — alg-confusion defence). Unblocked `pnpm test`: green across **15/15 tasks, 84 tests** (game-engine 40, shared 20, trading 9, auth 9, market 6). auth- & trading-services now build clean. Remaining services (portfolio/league/social/nft/education/notification), API gateway, and live web wiring still pending → full `pnpm build`/`typecheck` not yet green.
